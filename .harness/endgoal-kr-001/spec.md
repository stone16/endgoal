...
**TDD pattern**: All code in every checkpoint must be written test-first. The Generator writes the test, watches it fail, then writes the minimal implementation to make it pass. No exceptions.

**95%+ test coverage per checkpoint**: Each checkpoint must achieve ≥95% line coverage for all new code introduced in that checkpoint. Measured via:
- Rust: `cargo tarpaulin --workspace --out json` (or `cargo llvm-cov`)
- Frontend: `vitest --coverage` with c8/istanbul

Coverage is checked in acceptance criteria for every checkpoint. A checkpoint with <95% coverage is a FAIL verdict regardless of other criteria.

**E2E test per checkpoint**: Every checkpoint includes at least one end-to-end integration test that exercises the full path through that checkpoint's scope — not just unit tests of individual functions. Specifically:
- Backend checkpoints (CP02-06, CP10): integration test that starts the axum server, makes real HTTP calls, asserts real DB state
- Frontend checkpoints (CP07-09): Playwright or manual verification of the full user flow documented in output-summary.md
- Fullstack checkpoints (CP11-13): integration test covering frontend → backend → DB (or daemon) → response
- Infrastructure checkpoint (CP01): migration apply + ts-rs generate + build succeeds as the E2E
...
Every `POST /api/nodes/:id/freeze/respond` response is an SSE stream. Two event types:

**Proposal event** (agent proposes an item):
```json
{ "event_type": "proposal",
  "layer": "assertion" | "metric" | "rubric",
...
**Layer-complete event** (no more items in current layer):
```json
{ "event_type": "layer_complete",
  "layer": "assertion" | "metric" | "rubric",
  "next_layer": "metric" | "rubric" | null }
```
When `next_layer: null`, all layers have been proposed and the frontend shows "Review all & Commit".

Note: there is NO `layer: "done"` event. Use `event_type: "layer_complete"` with `next_layer: null` for the final state. This avoids ambiguity between per-layer completion and all-layers completion.
...
## Universal checkpoint requirements

Every checkpoint below inherits these requirements (not restated in each):

1. **TDD**: Tests written before implementation code. Output-summary.md must document the test-first sequence.
2. **≥95% coverage**: `cargo tarpaulin` (Rust) or `vitest --coverage` (frontend) must report ≥95% line coverage for new code. Evidence in evaluation.md.
3. **E2E test**: At least one integration/E2E test per checkpoint (type specified per checkpoint below).
4. **`cargo test --workspace` passes** (for all CPs after CP01).
5. **`pnpm typecheck` passes** (for all CPs after CP07).
...
- Scope: Implement freeze session backend entirely, no frontend yet. DB table `freeze_sessions` already in schema from CP01. Backend endpoints: `GET /api/nodes/:id/freeze/active` (returns active session `{ session_id, approved_items_json, current_layer }` or `null` if no active session — used by CP11 to detect resume vs. start), `POST /api/nodes/:id/freeze/start` (creates freeze_session row, returns `{ session_id }`; if an active session exists, marks it `abandoned` first), `POST /api/nodes/:id/freeze/respond` (accepts `{ session_id, user_response: string, action: "start" | "approve" | "edit" | "reject" | "skip_layer", approved_item_json?: string }` — `action: "start"` generates the first proposal for the current layer (used after `/start` or on resume); `action: "approve"/"edit"` appends `approved_item_json` to `freeze_sessions.approved_items_json` in DB; `action: "reject"` skips item; `action: "skip_layer"` advances `current_layer` in DB; all actions persist to DB before generating next proposal; returns SSE stream per the schema in Technical Approach: either `event_type: "proposal"` or `event_type: "layer_complete"`), `POST /api/nodes/:id/freeze/commit` (converts `approved_items_json` in freeze_session → `StructuredAcceptance`, writes to `nodes.acceptance_json`, transitions Node to Active if was Draft, marks session committed). State machine transitions: `active` → `committed` (on commit) or `abandoned` (on new freeze/start for same node). Proposal generation: prompt includes Node intent + docs + parent context + already-approved items. Generate assertions first (one `event_type: "proposal"` SSE event per item), then metrics, then rubric. After each layer is exhausted: emit `event_type: "layer_complete"` with `next_layer` pointing to the next layer (or `null` if final). Handle client disconnect: cancel `reqwest` stream (use `CancellationToken`).
